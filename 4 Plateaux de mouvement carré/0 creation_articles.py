...
def generate_html(data):
    ...
    <html lang="fr">
        ...
        <h2>Donnez Vie à Vos Champs de Bataille avec <strong>{titre}</strong> !</h2>
        <p>Plongez au cœur des combats urbains et des conflits post-apocalyptiques avec <strong>{titre}</strong>, une pièce maîtresse de notre gamme <strong>Ruines de la Ville</strong>, conçue pour les passionnés de jeux fantastiques comme <strong>Warhammer Age of Sigmar, The Old World, 9th Age, Kings of War </strong>, et bien d'autres !</p>
        <p>Réalisée avec une qualité d'impression 3D exceptionnelle, ce décor détaillé, déssiné par <strong>Makers Anvil</strong> ajoutera une dimension unique à vos champs de bataille. Que vous souhaitiez donner vie à une cité en ruines, créer un point stratégique pour vos armées ou simplement enrichir votre collection, <strong>{titre}</strong> est le complément idéal.</p>
