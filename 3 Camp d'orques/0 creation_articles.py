...
def generate_html(data):
    ...
    <html lang="fr">
        ...
        <h2>Donnez vie à vos armées fantastiques avec <strong>{titre}</strong> !</h2>
        <p>Plongez au cœur de l'action avec <strong>{titre}</strong>, une pièce maîtresse de notre gamme <strong>Orc Camp</strong>, conçue pour les passionnés de jeux fantastiques comme <strong>Warhammer Age of Sigmar, The Old World, 9th Age, Kings of War </strong>, et bien d'autres !</p>
        <p>Réalisée avec une qualité d'impression 3D exceptionnelle, ce décor détaillée, déssiné par <strong>Makers Anvil</strong>ajoutera une dimension unique à vos champs de bataille. Que vous souhaitiez organiser des combats épiques entre vos figurines, créer des décors immersifs pour vos dioramas ou simplement enrichir votre collection, <strong>{titre}</strong> est le complément idéal.</p>
